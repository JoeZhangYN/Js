...
//        样本量两道守卫（无证据→不跳）；③ bossImperil.when 仍保留 DOM skillReady["213"] 原始就绪条件。
...
const rules = read("battle/rules/index.js");

const fails = [];
const need = (cond, msg) => { if (!cond) fails.push(msg); };

// ① 学习器决策：toggle 门 + mid 缺失守卫 + 样本量守卫，缺一即可能无证据放跳
need(/skipImperilWhenOfcKills/.test(learner), "big-skill-kill-learner 缺 skipImperilWhenOfcKills 开关门");
...
need(/skillReady\["213"\]/.test(rules), "rules/index.js bossImperil.when 丢失 snap.skillReady[\"213\"] 原始就绪条件");
...
console.log("[verify-imperil-safety] OK — 跳 Imperil 全程门控 + 无证据保留 + DOM 就绪条件 三不变量在位");
