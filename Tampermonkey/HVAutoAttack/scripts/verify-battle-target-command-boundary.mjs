...
if (!clickTargetBody.includes("readLiveTarget(targetId)")) {
  violations.push(
    `${owner.replaceAll("\\", "/")} direct target clicks must use live-target ruling`
  );
}
if (/gE\(targetSelector\(targetId\)\)/.test(clickTargetBody)) {
  violations.push(
    `${owner.replaceAll("\\", "/")} direct target clicks must not bypass live-target ruling`
  );
}
...
requireText(liveTargetTest, ["rejects direct target clicks when the target is dead", "targetDead"]);
