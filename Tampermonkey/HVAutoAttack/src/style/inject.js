...
export function addStyle(lang) {
  ...
  const cssContent = [
    ...
    // 可拖拽调整宽高（resize:both + 最小限制）；内部菜单/面板/按钮改相对盒子的 absolute 填充，随盒子伸缩。
    '#hvAABox{left:calc(50% - 350px);top:50px;font-size:16px!important;z-index:4;width:700px;height:600px;min-width:520px;min-height:420px;resize:both;overflow:hidden;position:absolute;text-align:left;background-color:#E3E0D1;border:1px solid #000;border-radius:10px;font-family:"Microsoft Yahei";}',
    // tablist 绝对填充盒子中段（上让出表头，下让出按钮区）
    ".hvAATablist{position:absolute;left:14px;right:8px;top:44px;bottom:46px;}",
    // 左侧菜单固定 84px 宽（容 4 字）+ 自身纵向滚动（tab 多时不溢出，不再被内容面板裁切）
    ".hvAATabmenu{position:absolute;left:0;top:0;bottom:0;width:100px;overflow-y:auto;overflow-x:hidden;}",
    ".hvAATabmenu>span{display:block;padding:2px 8px;margin:0 0 1px 0;border:1px solid #91a7b4;border-radius:5px;background-color:#E3F1F8;color:#000;text-decoration:none;white-space:nowrap;text-overflow:ellipsis;overflow:hidden;cursor:pointer;}",
    ...
    ".hvAATab{position:absolute;left:108px;right:0;top:0;bottom:0;box-sizing:border-box;padding:15px;border:1px solid #91A7B4;border-radius:3px;box-shadow:0 2px 3px rgba(0,0,0,0.1);color:#666;background-color:#EDEBDF;overflow:auto;}",
