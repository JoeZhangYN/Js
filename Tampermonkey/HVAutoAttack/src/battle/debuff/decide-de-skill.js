// PURE: 单目标 debuff（useDeSkill）决策。从统一怪物视图(snap.view)派生目标，走 target strategy entry。
...
export function decideDeSkill(event = {}) {
  ...
  for (const key of skillPack) {
    ...
    const target = runBattleTargetStrategy({
      type: isDrainMaxHp
        ? BattleTargetStrategyEvent.HIGHEST_ABS_HP
        : BattleTargetStrategyEvent.FIRST_BY_ORDER,
      alive,
    });
    ...
    const targetId = isDrainMaxHp
      ? runBattleTargetStrategy({ type: BattleTargetStrategyEvent.SELF_TARGET, monster: target })
