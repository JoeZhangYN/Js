...
/** snap.monsters 形态怪物 */
const sm = (over = {}) => ({
  id: 1, order: 0, isDead: false, isBoss: false, name: "X", hpRatio: 1, buffs: [], buffEffects: [],
  ...over,
});
/** monsterStatus 形态记录（含 monsterId/level；库 join 现按 MID 而非 name） */
const st = (over = {}) => ({
  id: 1, order: 0, monsterId: 100, level: 365, isDead: false, hp: 1000, hpNow: 1000, finWeight: 1,
  ...over,
});
/** 满九抗库记录 */
const fullDb = (over = {}) => ({
  ...
  crushing: 5, slashing: -10, piercing: 0, lastUpdate: "2026-06-25",
  ...over,
});
...
describe("joinMonsterView", () => {
  ...
  it("只 maxHP 的库记录 → resists undefined 但 dbMaxHP 有（按 MID）", () => {
    const view = readView([sm()], [st({ monsterId: 300 })], { 300: { monsterId: 300, maxHP: 900 } });
    expect(view[0].resists).toBeUndefined();
...
describe("monsterHpVars", () => {
  ...
  it("多怪存活 → solo 退 100；lowest/first 生效(百分比)", () => {
    const view = [v({ order: 0, hpPercent: 0.8 }), v({ order: 1, hpPercent: 0.3 }), v({
      order: 2,
      hpPercent: 0.5,
    })];
