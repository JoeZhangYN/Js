...
describe("selfTarget", () => {
  it("恒返自身 id(无邻居偏移)", () => expect(selfTarget(m({ id: 7 }))).toBe(7));
});

describe("aoeNeighborAnchor", () => {
  const self = { id: 1 };
  it("AoE≥2 且邻居存活 → 邻居 id", () => {
    expect(aoeNeighborAnchor(self, { id: 2, isDead: false }, 2)).toBe(2);
  });
  it("AoE<2 → self", () => expect(aoeNeighborAnchor(self, { id: 2, isDead: false }, 1)).toBe(1));
  it("邻居已死 → self", () => expect(aoeNeighborAnchor(self, { id: 2, isDead: true }, 2)).toBe(1));
  it("无邻居 → self", () => expect(aoeNeighborAnchor(self, undefined, 2)).toBe(1));
});

describe("bossCoverageWindow", () => {
  const needy = (x) => x.isBoss && !x.buffs.includes("imperil");
  it("单 needy boss → 命中它", () => {
    const r = bossCoverageWindow([m({ id: 7, order: 0, isBoss: true })], 1, needy);
    expect(r.id).toBe(7);
  });
  it("aoe=2 相邻两 needy boss → click 后者覆盖 [前,后]", () => {
    const alive = [m({ id: 1, order: 0, isBoss: true }), m({ id: 2, order: 1, isBoss: true })];
    expect(bossCoverageWindow(alive, 2, needy).id).toBe(2);
  });
  it("aoe=1 两不相邻 needy → 选 order 较小", () => {
    ...
    expect(bossCoverageWindow(alive, 1, needy).id).toBe(1);
  });
  ...
  it("无 needy → null", () => {
    expect(bossCoverageWindow([m({ isBoss: false })], 2, needy)).toBeNull();
  });
});
