 ...
 */
export function dynamicHpThreshold(snap, opt) {
  const fallback = opt.hp1 ?? 50;
  if (!opt.dynamicHealThreshold) return fallback;
  if (!snap.playerIncomingDps || snap.playerIncomingDps.sampleCount < 2) {
    return fallback;
  }
  const turns = estimateRemainingTurns(snap);
  // CVaR 风格：用 perTurnP95 替 mean perTurn，触发更早 / 更保守。
  // 数据稀疏时 p95 退化为 mean（quantile 取最大值 ≈ mean），自然 fallback。
  const dpsEstimate = snap.playerIncomingDps.perTurnP95 ?? snap.playerIncomingDps.perTurn;
