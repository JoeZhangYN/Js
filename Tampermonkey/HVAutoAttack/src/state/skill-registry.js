// 技能登记表 SOT（从 cd-tracker.js 抽出 —— 打破 cd-tracker ↔ cd-learner 循环依赖）。
// 纯数据 / 纯函数，零模块依赖：cd-tracker 与 cd-learner 都向下 import 它，故无环（check-circular 绿）。
//
// 全 23 个 fixed-CD 技能登记：5 物理（盾战 T1/T2/T3 + OFC + FRD）+ 18 攻击魔法（6 元素 × 3 阶）。
// 双刀/双手流派的 T1-T3 ID 与盾战不同（21x1/2/3, 23x1/2/3），运行时由 fightingStyle 经 effectiveSkillId 解析。
// cdBase 是基础值——HV 用户 Ability 可能减半，本表保守取上限（cd-learner 据真机观测向下收敛）。
/**
