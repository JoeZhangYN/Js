// ==UserScript==
// @name        PriceForgedEquipment-Isekai
// @namespace   HVEquipPrice
// @description Calculate material cost for forged Legendary items only
// @match       http://alt.hentaiverse.org/isekai/equip/*
// @match       https://hentaiverse.org/isekai/equip/*
// @require
...
function outputSummary( baseSummary ){
    if( Object.keys(output).length > 0 ) {
        ...
        var mg = document.createElement('span'); mg.className = "ep";
        var hg = document.createElement('span'); hg.className = "ep";
        var bi = document.createElement('span'); bi.className = "ep";
        var ca = document.createElement('span'); ca.className = "ep";
        var sp = document.createElement('span'); sp.className = "ep";
        var tot = document.createElement('p'); tot.style = "color:#F00";

        mg.innerHTML = "Mid-Grade " + credit( summary.midGrade ); html.appendChild(mg);
        hg.innerHTML = " High-Grade " + credit( summary.highGrade ); html.appendChild(hg);
        bi.innerHTML = " Binding " + credit( summary.binding ); html.appendChild(bi);
        ca.innerHTML = "<br/> Catalyst " + credit( summary.catalyst ); html.appendChild(ca);
        sp.innerHTML = " Special " + credit( summary.special ); html.appendChild(sp);
        if( baseSummary ) {
            totalInvested.summary = summary;
            tot.innerHTML = "Total " + credit( upgradeTotal( summary ) ); html.appendChild(tot);
        } else {
            tot.innerHTML = "Total " + credit( upgradeTotal( summary ) ) + " (" + credit( upgradeTotal( totalInvested.summary ) ) + ")"; html.appendChild(tot);
        }
