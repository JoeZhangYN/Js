...
// @name         HV Utils 统一汉化
// @namespace    HVUT
// @version      4.0.0+isekai4.2.0
// @description  HV Utils 主世界 v4.0.0 + Isekai v4.2.0 统一汉化 (sssss2 原料, JoeZhangYN 合并/迁移; 运行时 IS_ISEKAI 分发)
// @author       sssss2, sake123456, JoeZhangYN
...
if (_query.s === 'Character' && _query.ss === 'in') {
    ...
    function load(e) {
      ...
      const tid = setInterval(() => {
        ...
        if (count > 100) {
          ...
          if (!div) {
            eq.node.note.value = "无法加载百分位区间脚本";
          }
          setTimeout(() => {
  ...
  _in.wts = function () {
    ...
    popup_text(code.trim() || '没有选中装备.', 900, 500);
  };
  ...
  if (_query.filter || !$config.settings.equipInventoryIntegration) {
    ...
    $input(['button', '排序：名称'], _in.node.side, null, () => { _in.sort_list('姓名'); });
    $input(['button', '排序：装备id'], _in.node.side, { className: 'hvut-side-margin' }, () => { _in.sort_list('eid'); });
...
if (_query.s === 'Bazaar' && _query.ss === 'es') {
  ...
  _es.sell_all = async function () {
    ...
    if ($config.settings.equipmentShopConfirm === 2 || $config.settings.equipmentShopConfirm === 1 && salvage.length || valuable.length) {
      let msg = `确定要出售这 ${selected.length} 件装备获取 ${sum.toLocaleString()} credits 吗?`;
      if (salvage.length) {
  ...
  _es.item_pane_calc = function (eq) {
    _es.salvage_calc(eq);
    eq.node.salvage.textContent = `分解估值 ${eq.data.salvage_value}`;
  ...
  _es.shop_pane_init = function (filter, outer) {
    ...
    equiplist.forEach((eq) => {
      ...
      eq.node.buy = $element('span', eq.node.sub, [`购买价 ${eq.data.value}`, { dataset: { action: 'buy', eid: eq.info.eid, pane: 'shop' } }]);
  ...
  toggle_button($input('button', _es.node.side, { className: 'hvut-side-margin' }), '显示所有装备', '使用过滤器', $id('shop_pane'), 'hvut-none-cont', $config.settings.equipmentShopBazaarFilters.length);
  $input(['button', '全选'], _es.node.side, null, () => { _es.select_all('sell'); });
  $input(['button', '出售选定装备'], _es.node.side, { className: 'hvut-side-margin' }, () => { _es.sell_all(); });
  $input(['button', '全选'], _es.node.side, null, () => { _es.select_all('salvage'); });
  $input(['button', '分解选定装备'], _es.node.side, { className: 'hvut-side-margin' }, () => { _es.salvage_all(); });
  $input(['button', '编辑过滤器'], _es.node.side, null, () => { $config.open('equipmentShopProtectFilters'); });
...
if (_query.s === 'Forge' && _query.ss === 'up') {
  ...
  if ($id('equip_extended')) {
    ...
    _up.upgrade_sum = function () {
      ...
      _up.node.costs.innerHTML = '<tr><td>材料</td><td>需求</td><td>库存</td><td>单价</td></tr>';
      const prices = $price.get('Materials');
    ...
    _up.upgrade_run = async function () {
      if (_up.upgrade_run.done || !_up.upgrade_sum() || !confirm('确定要强化这件装备吗?')) {
        return;
      }
...
if (_query.s === 'Forge' && _query.ss === 'sa') {
  ...
  if ($id('salvage_button')) {
    confirm_event($id('salvage_button').parentNode, 'click', '确定要分解这件装备吗?', () => $id('salvage_button').src.includes('salvage.png'));
  }
} else
